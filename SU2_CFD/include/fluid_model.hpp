...
protected:
  su2double StaticEnergy, /*!< \brief Internal Energy. */
    ...
    source_energy,        /*!< \brief energy source term for enthalpy scalar. */
    source_prog,          /*!< \brief chemical source term for progress variable scalar. */
    *scalar_table,        /*!< \brief Scalar values obtained from the flamelet table. */
    *source_scalar;       /*!< \brief source terms for other scalars. */
...
public:
   ...
   */
  virtual su2double GetMassDiffusivity();
  ...
  void SetDiffusivityState(su2double val_temperature, su2double val_Density,
                           su2double val_Mu, su2double val_Mu_Turb,
                           su2double val_Cp);

  /*!
   * \brief virtual member that would be different for each gas model
   * implemented \param[in] InputSpec - Input pair for FLP calls ("e, rho").
  ...
  /*!
   * \brief virtual member that would be different for each gas model
   * implemented \param[in] InputSpec - Input pair for FLP calls ("PT").
  ...
  /*!
   * \brief virtual member that would be different for each gas model
   * implemented \param[in] InputSpec - Input pair for FLP calls ("Pv").
   * \param[in] th1 - first thermodynamic variable (P).
  ...
  /*!
   * \brief virtual member that would be different for each gas model
   * implemented \param[in] InputSpec - Input pair for FLP calls ("hs").
  ...
  /*!
   * \brief virtual member that would be different for each gas model
   * implemented \param[in] InputSpec - Input pair for FLP calls ("rhoT").
  ...
  virtual void SetTDState_T(su2double val_temperature,
                            su2double *val_scalars = NULL);

  virtual void SetFluidFlameletTableOutput(CConfig *config,
                                           su2double val_temperature,
                                           su2double *val_scalars = NULL);

  /*!
   * \brief Set fluid eddy viscosity provided by a turbulence model needed for
   * computing effective thermal conductivity.
   */
  void SetEddyViscosity(su2double val_Mu_Turb);

  virtual su2double GetSourceEnergy();

  virtual su2double GetSourceProg();

  /*!
   * \brief Get the scalar values interpolated from the flamelet table.
   */
  virtual su2double *GetScalarTable();

  virtual su2double LookUp_ProgEnth(string val_name_var, su2double val_prog,
                                    su2double val_enth);

  virtual su2double GetEnthFromTemp(su2double val_prog, su2double val_temp);

  virtual pair<su2double, su2double> GetTableLimitsEnth();
  virtual pair<su2double, su2double> GetTableLimitsProg();
...
class CIdealGas : public CFluidModel {
 ...
 public:
  ...
  /*!
   * \brief compute some derivatives of enthalpy and entropy needed for subsonic
   * inflow BC \param[in] InputSpec - Input pair for FLP calls ("Pv").
   * \param[in] th1 - first thermodynamic variable (P). \param[in] th2 - second
   * thermodynamic variable (v).
   *
   */
...
class CVanDerWaalsGas : public CIdealGas {
 protected:
  su2double a, b, Zed; /*!< \brief Parameters for the Dimensionless Equation. */
 ...
 public:
   ...
   * \brief Constructor of the class.
   */
  CVanDerWaalsGas(su2double gamma, su2double R, su2double Pstar,
                  su2double Tstar);
...
public:
   ...
   * \brief Constructor of the class.
   */
  CPengRobinson(su2double gamma, su2double R, su2double Pstar, su2double Tstar,
                su2double w);
 ...
 * \brief Child class for defining a constant density gas model (incompressible
 * only). \author: T. Economon
 */
...
public:
   ...
   * \param[in] T - Temperature value at the point.
   */
  void SetTDState_T(su2double val_temperature, su2double *val_scalars = NULL);
};
...
public:
   ...
   * \brief Constructor of the class.
   */
  CIncIdealGas(su2double val_Cp, su2double val_gas_constant,
               su2double val_operating_pressure);
...
protected:
  unsigned short nPolyCoeffs; /*!< \brief Number of coefficients in the
                                 temperature polynomial. */
  su2double Gas_Constant,     /*!< \brief Specific Gas Constant. */
      *b,    /*!< \brief Polynomial coefficients for Cp as a function of
                temperature. */
...
public:
   ...
   * \brief Constructor of the class.
   */
  CIncIdealGasPolynomial(su2double val_gas_constant,
                         su2double val_operating_pressure);
