...
CCentJST_Flow::CCentJST_Flow(unsigned short val_nDim, unsigned short val_nVar, CConfig *config) :
  CNumerics(val_nDim, val_nVar, config), Velocity_i(NULL,0), Velocity_j(NULL,0) {
  ...
  Diff_U.resize(nVar);
  Diff_Lapl.resize(nVar);
  MeanVelocity.resize(nVar);
  ProjFlux.resize(nVar);
  U_i.resize(nVar);
  U_j.resize(nVar);
...
void CCentJST_Flow::ComputeResidual(su2double *val_residual, su2double **val_Jacobian_i, su2double **val_Jacobian_j,
  ...
  /*--- Map residual so it can be manipulated as an Eigen vector, this cannot be done for the Jacobians
        because ** is not compatible with compact storage (*), temporaries are used for those. ---*/

  SU2_ALG::CMapVecX Residual(val_residual,nVar);
  ...
  /*--- Map the part of V_ij corresponding to velocities ---*/
  new (&Velocity_i) SU2_ALG::CMapVecX(V_i+1,nDim);
  new (&Velocity_j) SU2_ALG::CMapVecX(V_j+1,nDim);
  
  sq_vel_i = 0.5*Velocity_i.squaredNorm();
  sq_vel_j = 0.5*Velocity_j.squaredNorm();

  /*--- Recompute conservative variables ---*/
  
  U_i(0) = 1.0;                      U_j(0) = 1.0;
  U_i.segment(1,nDim) = Velocity_i;  U_j.segment(1,nDim) = Velocity_j;
  U_i(nDim+1) = Energy_i;            U_j(nDim+1) = Energy_j;
  U_i *= Density_i;                  U_j *= Density_j;
  ...
  MeanVelocity = 0.5*(Velocity_i+Velocity_j);
  MeanEnergy   = 0.5*(  Energy_i+  Energy_j);
  
  /*--- Get projected flux tensor ---*/
  
  GetInviscidProjFlux(&MeanDensity, MeanVelocity.data(), &MeanPressure, &MeanEnthalpy, Normal, ProjFlux.data());
  ...
  if (implicit) {
    GetInviscidProjJac(MeanVelocity.data(), &MeanEnergy, Normal, 0.5, val_Jacobian_i);
    for (iVar = 0; iVar < nVar; iVar++)
      for (jVar = 0; jVar < nVar; jVar++)
        Jacobian_i(iVar,jVar) = val_Jacobian_i[iVar][jVar];
  ...
  SU2_ALG::CMapVecX normal(Normal,nDim);
  
  if (grid_movement) {
    SU2_ALG::CMapVecX gridVel_i(GridVel_i,nDim), gridVel_j(GridVel_j,nDim);
        
    ProjGridVel = 0.5*(gridVel_i+gridVel_j).dot(normal);
    
    Residual -= ProjGridVel*0.5*(U_i+U_j);
    if (implicit) {
      Jacobian_i.diagonal().array() -= 0.5*ProjGridVel;
      Jacobian_j.diagonal().array() -= 0.5*ProjGridVel;
    }
  }
  ...
  Diff_Lapl = SU2_ALG::CMapVecX(Und_Lapl_i,nVar) - SU2_ALG::CMapVecX(Und_Lapl_j,nVar);
  Diff_U = U_i - U_j;
  Diff_U(nVar-1) = Density_i*Enthalpy_i - Density_j*Enthalpy_j;
  
  /*--- Compute the local spectral radius and the stretching factor ---*/
  
  ProjVelocity_i = Velocity_i.dot(normal);
  ProjVelocity_j = Velocity_j.dot(normal);
  Area = normal.norm();
  ...
  Residual += (Epsilon_2*Diff_U - Epsilon_4*Diff_Lapl)*StretchingFactor*MeanLambda;
  ...
  if (implicit) {
    ...
    Jacobian_i.diagonal().head(nVar-1).array() += cte_0;
    Jacobian_j.diagonal().head(nVar-1).array() -= cte_1;
    
    /*--- Last row of Jacobian_i ---*/
    
    Jacobian_i(nVar-1,0)              += cte_0*Gamma_Minus_One*sq_vel_i;
    Jacobian_i.block(nVar-1,1,1,nDim) -= cte_0*Gamma_Minus_One*Velocity_i.transpose();
    Jacobian_i(nVar-1,nVar-1)         += cte_0*Gamma;
    
    /*--- Last row of Jacobian_j ---*/
    
    Jacobian_j(nVar-1,0)              -= cte_1*Gamma_Minus_One*sq_vel_j;
    Jacobian_j.block(nVar-1,1,1,nDim) += cte_1*Gamma_Minus_One*Velocity_j.transpose();
    Jacobian_j(nVar-1,nVar-1)         -= cte_1*Gamma;
    
    /*--- For now we need to copy them ---*/
    
    for (iVar = 0; iVar < nVar; iVar++) {
      for (jVar = 0; jVar < nVar; jVar++) {
        val_Jacobian_i[iVar][jVar] = Jacobian_i(iVar,jVar);
        val_Jacobian_j[iVar][jVar] = Jacobian_j(iVar,jVar);
      }
    }
  }
