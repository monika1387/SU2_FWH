...
void CTNE2EulerSolver::SetNondimensionalization(CGeometry *geometry, CConfig *config, unsigned short iMesh) {
 ...
 for (iSpecies = 0; iSpecies < nHeavy; iSpecies++) {
  rhoCvtr += Density_FreeStream*MassFrac_Inf[iSpecies]*
    FluidModel->Calc_CvTraRotSpecies(Ru, iSpecies);
 }
...
void CTNE2EulerSolver::BC_Inlet(CGeometry *geometry, CSolver **solution_container,
 ...
 su2double P_Total, T_Total, Velocity[3], Velocity2, H_Total, Temperature, Temperature_ve, Riemann,
   Eve= 0.0, Pressure, Density, Energy, *Flow_Dir, Mach2, SoundSpeed2, SoundSpeed_Total2, Vel_Mag,
   alpha, aa, bb, cc, dd, Area, UnitNormal[3];
 su2double H,A, rcvve=0.0, rcvtr=0.0;
 bool implicit             = (config->GetKind_TimeIntScheme_TNE2() == EULER_IMPLICIT);
 bool grid_movement        = config->GetGrid_Movement();
 ...
 su2double *Normal    = new su2double[nDim];
 su2double *Mass_Frac = new su2double[nSpecies];
 su2double *cs        = new su2double[nSpecies];

 /*--- Pass indices to numerics ---*/
 conv_numerics->SetRhosIndex     ( node[0]->GetRhosIndex()      );
 conv_numerics->SetRhoIndex      ( node[0]->GetRhoIndex()       );
 conv_numerics->SetPIndex        ( node[0]->GetPIndex()         );
 conv_numerics->SetTIndex        ( node[0]->GetTIndex()         );
 ...
 unsigned short VEL_INDEX     = node[0]->GetVelIndex();
 unsigned short P_INDEX       = node[0]->GetPIndex();
 unsigned short RHO_INDEX     = node[0]->GetRhoIndex();
 ...
 for (iVertex = 0; iVertex < geometry->nVertex[val_marker]; iVertex++) {
  ...
  if (geometry->node[iPoint]->GetDomain()) {
   ...
   case TOTAL_CONDITIONS:{
    ...
    H_Total     = V_domain[H_INDEX];
    SoundSpeed2 = pow(V_domain[A_INDEX],2);

    /*--- Compute Species Concentrations ---*/
    for (iSpecies =0; iSpecies<nSpecies;iSpecies++){
     Mass_Frac[iSpecies] = V_domain[iSpecies]/Density;
    }
    ...
    for (iSpecies =0; iSpecies<nSpecies; iSpecies++){
     cs[iSpecies]= Mass_Frac[iSpecies]*Density;
    }

    /*--- Using pressure, density, & velocity, compute the energy ---*/
    Energy = Pressure/(Density*Gamma_Minus_One)+0.5*Velocity2;
    for (iSpecies =0; iSpecies<nSpecies; iSpecies++){
     Eve += FluidModel->CalcEve(Temperature_ve,iSpecies);
    }

    /*--- Conservative variables, using the derived quantities ---*/
    for (iSpecies=0; iSpecies<nSpecies; iSpecies++)
     U_inlet[iSpecies] = cs[iSpecies];
    for (iDim = 0; iDim < nDim; iDim++)
     U_inlet[nSpecies+iDim] = Velocity[iDim]*Density;
    U_inlet[nVar-2] = Energy*Density;
    U_inlet[nVar-1] = Eve*Density;


    /*---Compute Enthalpy, Cvs and Sound Speed ---*/
    su2double RuSI   = UNIVERSAL_GAS_CONSTANT;    // Universal gas constant [J/(mol*K)]
    su2double Ru     = 1000.0*RuSI;               // Universal gas constant [J/(kmol*K)]
    H = Energy+Pressure/Density;
    A = sqrt(SoundSpeed2);
    for (iSpecies=0; iSpecies<nSpecies; iSpecies++){
     rcvve += U_inlet[iSpecies]*FluidModel->Calc_CvVibElSpecies (Temperature_ve,iSpecies);
     rcvtr += U_inlet[iSpecies]*FluidModel->Calc_CvTraRotSpecies(Ru, iSpecies);
    }
    ...
    V_inlet[H_INDEX]   = H;
    V_inlet[A_INDEX]   = A;
    V_inlet[RHOCVTR_INDEX] = rcvtr;
    V_inlet[RHOCVVE_INDEX] = rcvve;
   ...
   //        visc_numerics->SetConservative(U_domain, U_inlet);
   //        visc_numerics->SetConsVarGradient(node[iPoint]->GetGradient(),
   //                                          node_infty->GetGradient() );
...
void CTNE2EulerSolver::BC_Supersonic_Inlet(CGeometry *geometry, CSolver **solution_container,
 ...
 unsigned long iVertex, iPoint, Point_Normal;
 su2double Density, Pressure, Temperature, Temperature_ve, *Velocity, *Mass_Frac, Velocity2, soundspeed;
 ...
 /*--- Initialize inlet vectors to zero ---*/
 for (iVar=0; iVar<nVar;     iVar++) U_inlet[iVar] = 0.0;
 for (iVar=0; iVar<nPrimVar; iVar++) V_inlet[iVar] = 0.0;

 /*--- Check for ionization in the flow ---*/
 bool ionization = FluidModel->GetIonization();
 if (ionization) { nHeavy = nSpecies-1; nEl = 1; }
 else            { nHeavy = nSpecies; nEl = 0; }

 /*--- Extract values from FluidModel [MAYBE UNECCESARY]---*/
 Ms = FluidModel->GetMolar_Mass();

 /*--- Supersonic inlet flow: there are no outgoing characteristics,
   so all flow variables can be imposed at the inlet.
   First, retrieve the specified values for the primitive variables. ---*/
 //Delete me...TEMP_VE is set to temp atm
 Mass_Frac      = FluidModel->GetMassFrac_FreeStream();
 ...
 /*--- Compute Density ---*/
 Density = FluidModel->Calc_Density(Mass_Frac, Temperature,
                                    Temperature_ve, Pressure);

 /*--- Compute Soundspeed and Velocity squared ---*/
 for (iSpecies = 0; iSpecies < nHeavy; iSpecies++) {
  rhoCvtr += Density*Mass_Frac[iSpecies]*
    FluidModel->Calc_CvTraRotSpecies(Ru, iSpecies);
 }
 soundspeed = FluidModel -> Calc_SoundSpeed(Mass_Frac, rhoCvtr,
                                            Density, Pressure);
 ...
 Temperature_ve = Temperature_ve/config->GetTemperature_Ref();
 ...
 rhoE   = FluidModel->Calc_MixtureEnergy(Mass_Frac, Velocity2, Density,
                                         Temperature, Temperature_ve);

 /*--- Compute energy of vibrational-electronic mode ---*/
 for (iSpecies = 0; iSpecies<nSpecies; iSpecies++){
  rhoEve += (Mass_Frac[iSpecies]*Density)*
    FluidModel->CalcEve(Temperature_ve,iSpecies);
 }

 /*--- Compute CV of vibrational-electronic mode ---*/
 for (iSpecies = 0; iSpecies<nSpecies; iSpecies++){
  rhoCvve += (Mass_Frac[iSpecies]*Density)*
    FluidModel->Calc_CvVibElSpecies(Temperature_ve,iSpecies);
 }

 /*--- Setting inlet conservative variables ---*/
 for (iSpecies = 0; iSpecies < nSpecies; iSpecies++)
 ...
 for(iVertex = 0; iVertex < geometry->nVertex[val_marker]; iVertex++) {
  ...
  if (geometry->node[iPoint]->GetDomain()) {
   ...
   /*--- Current solution at this boundary node ---*/
   for (iVar = 0; iVar < nVar; iVar++)     U_domain[iVar] = U_inlet[iVar];
   for (iVar = 0; iVar < nPrimVar; iVar++) V_domain[iVar] = V_inlet[iVar];
...
void CTNE2NSSolver::Friction_Forces(CGeometry *geometry, CConfig *config) {
 ...
 unsigned short RHO_INDEX,VEL_INDEX, T_INDEX, TVE_INDEX, LAM_VISC_INDEX, K_INDEX, KVE_INDEX;
